...
public class CsvAuditEventHandlerReadBenchmarkTest extends BenchmarkBase {
    ...
    public QueryResponse queryForId(final ReadState state) throws Exception {
        ...
        final QueryRequest queryRequest = Requests.newQueryRequest(ACCESS)
                .setQueryFilter(QueryFilters.parse("/_id eq \"" + queryHandler.id + "\""));
     ...
     */
    private static class IdentifierQueryResourceHandler implements QueryResourceHandler {
        final String id;

        IdentifierQueryResourceHandler(final String id) {
            this.id = id;
        }
